...
<p align="center">
  <img src="src/assets/logo.svg" width="128" height="128" alt="Dclaw Logo" />
</p>

<h1 align="center">Dclaw</h1>
...
<p align="center">
  ...
  <img src="https://img.shields.io/github/downloads/ValueCell-ai/Dclaw/total?color=%23027DEB" alt="Downloads" />
...
**Dclaw** bridges the gap between powerful AI agents and everyday users. Built on top of [OpenClaw](https://github.com/OpenClaw), it transforms command-line AI orchestration into an accessible, beautiful desktop experience—no terminal required.

Whether you're automating workflows, managing AI-powered channels, or scheduling intelligent tasks, Dclaw provides the interface you need to harness AI agents effectively.

Dclaw comes pre-configured with best-practice model providers and natively supports Windows as well as multi-language settings. Of course, you can also fine-tune advanced configurations via **Settings → Advanced → Developer Mode**.
...
Building AI agents shouldn't require mastering the command line. Dclaw was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**

| Challenge | Dclaw Solution |
|-----------|----------------|
...
Dclaw is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
When you target another agent with `@agent`, Dclaw switches into that agent's own conversation context directly instead of relaying through the default agent. Agent workspaces stay separate by default, and stronger isolation depends on OpenClaw sandbox settings.
...
For custom channel account IDs, Dclaw enforces OpenClaw-compatible canonical IDs (`[a-z0-9_-]`, lowercase, max 64 chars, must start with a letter/number) to prevent routing mismatches.
Dclaw now also bundles Tencent's official personal WeChat channel plugin, so you can link WeChat directly from the Channels page with an in-app QR flow.
...
Dclaw also pre-bundles full document-processing skills (`pdf`, `xlsx`, `docx`, `pptx`), deploys them automatically to the managed skills directory (default `~/.openclaw/skills`) on startup, and enables them by default on first install. Additional bundled skills (`find-skills`, `self-improving-agent`, `tavily-search`) are also enabled by default; if required API keys are missing, OpenClaw will surface configuration errors in runtime.  
...
When a compatible gateway rejects `/models` for non-auth reasons, Dclaw automatically falls back to a lightweight `/chat/completions` or `/responses` probe during API key validation.

### 🌙 Adaptive Theming
Light mode, dark mode, or system-synchronized themes. Dclaw adapts to your preferences automatically.

### 🚀 Startup Launch Control
In **Settings → General**, you can enable **Launch at system startup** so Dclaw starts automatically after login.
...
Download the latest release for your platform from the [Releases](https://github.com/ValueCell-ai/Dclaw/releases) page.

#### Build from Source

```bash
# Clone the repository
git clone https://github.com/ValueCell-ai/Dclaw.git
cd Dclaw
...
When you launch Dclaw for the first time, the **Setup Wizard** will guide you through:
...
> Note for Moonshot (Kimi): Dclaw keeps Kimi web search enabled by default.  
> When Moonshot is configured, Dclaw also syncs Kimi web search to the China endpoint (`https://api.moonshot.cn/v1`) in OpenClaw config.

### Proxy Settings

Dclaw includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
- If advanced proxy fields are left empty, Dclaw falls back to `Proxy Server`.
- Saving proxy settings reapplies Electron networking immediately and restarts the Gateway automatically.
- Dclaw also syncs the proxy to OpenClaw's Telegram channel config when Telegram is enabled.
- Gateway restarts preserve an existing Telegram channel proxy if Dclaw proxy is currently disabled.
...
Dclaw employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:
...
- Dclaw is an Electron app, so **one app instance normally appears as multiple OS processes** (main/renderer/zygote/utility). This is expected.
...
- Clicking the window close button (`X`) hides Dclaw to tray; it does **not** fully quit the app. Use tray menu **Quit Dclaw** for complete shutdown.
...
- uses a temporary Dclaw `userData` directory
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make Dclaw better.
...
Dclaw is built on the shoulders of excellent open-source projects:
...
### Dclaw Partner Program 🚀

We're launching the Dclaw Partner Program and looking for partners who can help introduce Dclaw to more clients, especially those with custom AI agent or automation needs.

Partners help connect us with potential users and projects, while the Dclaw team provides full technical support, customization, and integration.
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/Dclaw&type=Date" alt="Star History Chart" />
</p>
...
Dclaw is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.
