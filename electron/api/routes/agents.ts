...
import { ensureDclawContext } from '../../utils/openclaw-workspace';
    ...
    if (pid) {
      ...
      try {
        if (process.platform === 'darwin' || process.platform === 'linux') {
          // MUST use -sTCP:LISTEN. Otherwise lsof returns the client process (Dclaw itself) 
...
export async function handleAgentRoutes(
  ...
  if (url.pathname === '/api/agents' && req.method === 'POST') {
    try {
      ...
      // Ensure newly provisioned workspaces get Dclaw context merge/cleanup
      // even when gateway status events do not fire (e.g. in-process reload).
      void ensureDclawContext({ waitForAllConfiguredWorkspaces: true }).catch((err) => {
        console.warn('[agents] Failed to ensure Dclaw context after agent creation:', err);
      });
