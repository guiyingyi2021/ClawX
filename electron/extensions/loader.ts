...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'dclaw-extensions.json');
  }
  return join(app.getAppPath(), 'dclaw-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No dclaw-extensions.json found, loading all builtin extensions');
  }
