...
import { ensureDclawContext, repairDclawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.dclaw.desktop';
...
// On Wayland this maps the running window to dclaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('dclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[Dclaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[Dclaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[Dclaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== Dclaw Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain Dclaw markers (no OpenClaw
  // template content). This fixes a race condition where ensureDclawContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairDclawOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running' && !isE2EMode) {
      void ensureDclawContext().catch((error) => {
        logger.warn('Failed to re-merge Dclaw context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge Dclaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureDclawContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureDclawContext().catch((error) => {
      logger.warn('Failed to merge Dclaw context into workspace:', error);
    });
  }
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second Dclaw instance detected; redirecting to the existing window');
