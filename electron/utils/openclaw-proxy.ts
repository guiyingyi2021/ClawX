...
interface SyncProxyOptions {
   ...
   * currently disabled in Dclaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync Dclaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because Dclaw proxy is disabled and preserve mode is enabled');
      return;
    }
