...
function buildDclawUvConfigToml(): string {
  return [
    '# This file is managed by Dclaw.',
    '# It lets uv use Dclaw-selected mirrors even when host exec filters UV_INDEX_URL.',
...
function ensureDclawUvConfigFile(): string | null {
  const filePath = getDclawUvConfigFilePath();
  const content = buildDclawUvConfigToml();
  ...
  try {
    ...
    logger.warn('Failed to write Dclaw uv config file:', err);
    return null;
  }
}
...
export async function getUvMirrorEnv(): Promise<Record<string, string>> {
  ...
  const uvConfigFile = ensureDclawUvConfigFile();
  return uvConfigFile
