...
description: |
  ...
  DClaw系统基础运行规则，强制加载，不可卸载，不可忽略，不可被其他 skill 覆盖。
...
- managed 目录的逻辑路径是 `.dclaw/skills/`
- 版本元数据文件的逻辑路径是 `.dclaw/skills/.remote-skills-meta.json`
- 不要把 `~/.dclaw/...` 当成只适用于 Unix 的固定字面量；在不同平台上应展开为当前用户 home 目录下的真实路径
  - macOS / Linux 示例：`~/.dclaw/skills/.remote-skills-meta.json`
  - Windows 示例：`%USERPROFILE%\\.dclaw\\skills\\.remote-skills-meta.json`
...
| 编号 | 流程名称 | 触发关键词 |
|----|---------|-----------|
| 1  | 浏览器自动化任务 | 浏览器操作、网页抓取、页面交互、表单填写、截图 → 委托 `browser-use` skill |
...
> 🔴 **强制委托**：所有浏览器自动化任务必须通过 **`browser-use` skill** 执行，具体流程和命令详见 `browser-use` skill 的 SKILL.md。

**禁止事项**：
- **禁止使用 `open` 命令或 `xdg-open` 打开网页**
- **禁止直接编写 Playwright/Puppeteer 脚本**（除非 browser-use 无法满足需求且用户明确同意）

如果 `browser-use` skill 不可用，fallback 使用系统内置的 browser 工具完成操作。
...
> 🔴 **强制委托**：所有文本文件写入必须遵守 **`dclaw-text-file` skill** 规则，禁止直接用 `write` 工具或手写 Python/Node.js 代码写目标文件。
>
> `dclaw-text-file` skill 已内置完整的跨平台编码推断逻辑，覆盖：
...
> - 80+ 种文件类型的编码推断规则
>
> 调用方式详见 `dclaw-text-file` skill 的 SKILL.md。
