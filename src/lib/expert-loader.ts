...
import EXPERTS_INDEX from '@/data/experts-index.json';
...
/**
 * 加载专家配置（混合模式）
 * 策略：立即返回内置专家 + 本地索引数据，后台异步检查远程更新
 * 与 market-loader.ts 策略完全一致：本地索引兜底，不依赖网络
...
export async function loadExperts(
  ...
  fetchRemoteExperts(url)
    .then(remote => {
      ...
      const hasNew = remote.experts.some(e => !existingIds.has(e.id));
      const hasMore = remote.experts.length !== immediateData.length;

      if (!hasNew && !hasMore) {
        // 无变化，仅刷新缓存时间戳
        const cache = getCache();
        if (cache) setCache(immediateData, cache.version);
        return;
      }
...
/**
 * 将本地索引文件（按分类组织）转换为 Expert[]
 * 与 market-loader.ts 的 transformMarketExperts 对齐
 */
function transformLocalExperts(): Expert[] {
  const flat: Expert[] = [];
  for (const [category, experts] of Object.entries(EXPERTS_INDEX as Record<string, any[]>)) {
    ...
    for (const expert of experts) {
      ...
      flat.push({
        ...expert,
        color: hexToTailwindGradient(expert.color),
        category,
        isRemote: true,
        soulUrl,
        downloadUrl: soulUrl || `${GITHUB_RAW_BASE}/${basePath}/package.tar.gz`,
